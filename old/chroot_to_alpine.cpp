...
void unsharenamespaces() {
    // const int UNSHARE_FLAGS = CLONE_NEWUTS | CLONE_NEWPID | CLONE_NEWNET | CLONE_FS | CLONE_FILES | CLONE_NEWCGROUP ;
    const int UNSHARE_FLAGS = CLONE_NEWUTS | CLONE_NEWPID | CLONE_NEWNET | CLONE_FS | CLONE_FILES | CLONE_NEWCGROUP ;
...
void doqueru(char* exec_path, char** argv) {
    ...
    char* const pttr = "/bin/sh";
    int status;
    if(child_pid == 0) {
        ASSERTMSG(-1 != (child_pid = fork()), "fork PID 2 has failed");
        // chdir(exec_path);
        if(child_pid == 0) {
            // ASSERTMSG(0 != chroot(exec_path), "chroot has failed");
            ASSERTMSG(-1 != execvp(exec_path, &argv[1]), "exec has failed");
        }
